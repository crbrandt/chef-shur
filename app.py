...
for i in range(0, len(df_full['Team_x'])):
    ...
    df_full.iat[i,10] = str(round(df_full.iat[i,10],2))
...
##Updaating Page Logo and Tab Title
...
with col_title:
  st.title('NFL Game Predictor')
  st.markdown(' ## Created by Cole Brandt')
  st.markdown('  Last updated: Saturday, November 6th, 2021')  
...
final = preds[['Visitor', 'Home', 'Game Time', 'Weather', 'Team_Spread', 'Model Prediction', 'Spread_Beater', 'Probability' ]]
final = final.rename(columns = {'Team_Spread': 'Spread', 'Spread_Beater': 'Winner Against Spread', 'Confidence'})
